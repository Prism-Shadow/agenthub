...
- `EventType` 改为 `"delta" | "stop"`，并新增 `UniDeltaEvent` 与 `UniStopEvent`，作为流所产出的 `UniEvent` 的收窄形态：一次流由任意多个 `UniDeltaEvent` 事件加上恰好一个 `UniStopEvent` 组成，`stop` 事件总在最后。`delta` 事件恰好携带一个内容项，其 `usage_metadata` 与 `finish_reason` 为空；`stop` 事件不携带内容项，其 `usage_metadata` 与 `finish_reason` 均非空。`start` 与 `unused` 事件类型被移除。
...
- 新增 `StreamProtocolError`（携带 `client` 的 `AgentHubError`），并从包中导出。客户端的输出违反语法时，它在任何模式下都会抛出：内容项缺少 `fidelity.item_id`、内容项完成后又来片段、同一内容项中出现第二个不同的 fidelity、第一个 `tool_call.delta` 缺少名称或 id、某个内容项的 id 下出现另一种类型的片段。
...
- `transform_model_output_to_uni_event` / `transformModelOutputToUniEvent` 与 `_streaming_response_internal` / `_streamingResponseInternal` 沿用 0.4.x 的名称。客户端把每个线路事件转换为一个通用形态的 `UniEvent`，其内容项为 `.delta` 片段与 done 标记，基类再由这些事件构建流。没有任何通用内容的线路事件返回空的 delta 事件：不含内容项，`usage_metadata` 与 `finish_reason` 为空。
- 每个 `.delta` 项都以 `fidelity.item_id` 携带服务商为其内容项给出的标识；`embedding.delta` 项不携带，因为每个向量本身即完整。内容项在其 done 标记处结束（`done_marker` / `doneMarker`：一个 fidelity 只含 `item_id` 的 `text.done`），或在服务商的流结束时结束。
- `usage_metadata` 与 `finish_reason` 在线路给出它们的位置随 `stop` 事件到达，可以分段到达，由基类逐字段合并；客户端的 `stop` 事件不会结束流。
- 基类把事件组装成流：先剥离 `item_id`，再拼接片段、解析工具调用参数、在服务商的流结束时关闭仍未完成的内容项，并构造 `stop` 事件，因此 `item_id` 不会到达消费方、trace 或历史记录。各客户端中的工具调用累加器、参数解析、用量合并与合成 stop 的逻辑被移除，基类中针对 `unused` 事件的防护也一并移除。
- 内容项 id：`claude5` 与 `ant_messages` 使用 content block 的 index；`gpt6`、`openai_responses`、`deepseek_v4` 与 `minimax_m3` 使用输出 item 的 id；`openai_chat`、`openai_chat_vllm_adapter`、`glm5_3` 与 `kimi_k3` 使用序号，流式输出的种类变化或新的工具调用开始时序号递增，同时关闭前一个内容项；`gemini3_8` 使用 step 的 index 加上该 step 内同一内容种类连续段的序号（`<index>.<run>`）；`openai_embedding` 与 `gemini3_8` 的 embedding 向量不使用 id。
