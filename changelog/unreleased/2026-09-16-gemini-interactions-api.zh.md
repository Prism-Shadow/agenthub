...
- 含有 `thought` step 却不以其开头的助手消息（图像模型有时在第一个思考之前先流出文本），回放时以一个携带占位 signature `skip_thought_signature_validator` 的 `thought` step 开头；含有文本、媒体或工具调用、却没有携带 signature 的 `thought` step 的助手消息（由其他提供方生成的轮次）同样如此。
