...
- **Clients return parts; the base class builds the stream.** `transform_model_output_to_client_parts` / `transformModelOutputToClientParts` turns one wire event into a list of `ClientPart`s (`base_client.py` / `baseClient.ts`), and `_streaming_response_internal` builds the request, opens the stream, and yields each event's parts, adding only the key bookkeeping described below where the wire lacks item identity. A part is `delta` (one `.delta` item under a `key`), `done` (the item under `key` is complete), or `finish` (`usage_metadata` and/or `finish_reason`, yielded wherever the wire reports them — Anthropic `message_start` and `message_delta`, the Chat Completions finish and usage chunks, `response.completed` — and merged field by field by the base). The base class does the rest: it concatenates fragments into each `.done` item, parses tool-call arguments, keeps items from interleaving, closes items still open when the wire stream ends, and emits the one stop event or raises `EmptyResponseError`. A client therefore holds no accumulator, open-call map, argument parsing, or usage merging, and never synthesizes a stop. Parts that break the protocol raise `StreamProtocolError` in every mode, and the shared e2e tests check every client's stream with `assert_stream_grammar` / `assertStreamGrammar`.
- **A key is the provider's identity for the item, used once.** Stringify it: the Anthropic block `index`, the Responses `item_id` / `item.id` (falling back to `call_id` where a gateway omits the item id; a part with no id at all joins the item announced or streamed last while that item is open and of the same kind, and otherwise gets a fresh key), `embedding:<i>` for the i-th vector. Yield `done` where the provider signals that the item ended (`content_block_stop`, `response.output_item.done`). A key is never reused after its `done`: reasoning, then content, then reasoning again is three keys. Wire events a client knows but has nothing universal for are listed explicitly and return `[]`; anything else hits the unknown-event guard, which raises under `AGENTHUB_DEBUG` and returns `[]` otherwise.
- **Chat Completions mints its keys and closes on switch.** The protocol carries no item identity, so the client numbers items with a sequence that advances whenever the streamed kind changes (reasoning ↔ content ↔ a tool call) or a new tool call starts (a tool-call delta carrying a name, which every server sends only on a call's first chunk). It carries no end-of-item signal either, so the client remembers the key it streamed last and yields `done` for it before the first delta under a different key.
- **Fidelity goes out once, when it is complete.** Attach it to the delta at which the client knows it whole: Anthropic's signature on its `signature_delta`, as a `thinking.delta` with empty `thinking`; the Responses reasoning `channel` / `encrypted_content` on an empty `thinking.delta` yielded at `response.output_item.done`, right before that item's `done`; GPT's `phase` on an empty `text.delta` at `response.output_item.added`. The base copies it onto the `.done` item. An identical fidelity repeated on later deltas of the same item is dropped (Chat Completions tags every reasoning delta with `reasoning_field`); a different one is a `StreamProtocolError`.
- **Stream on deltas; never re-read the completed item.** A tool call opens with a `tool_call.delta` carrying the name and call id on the item-added event and continues with argument fragments; the completion event yields only the `done`, plus any fidelity unknown until then. Never read content back from a completed item or cross-check it against what the deltas produced; `response.function_call_arguments.done` returns `[]`. Leave provider error events (`response.failed`, `response.error`, `error`) to the unknown-event guard rather than translating them into AgentHub errors. `minimax_m3` is the deliberate exception: it ignores the argument deltas and reads each call from `response.output_item.done`, yielding one `tool_call.delta` with the name, call id, and whole arguments, then `done`, both under the item id. A client streams a call on deltas or delivers the completed item, never both, so the fragments and the complete call can never disagree.
