...
- `partial_tool_call`: Streaming tool-call fragment with `name`, partial JSON `arguments`, `tool_call_id`, and, when the provider streams one, the `item_id` of the call the fragment belongs to.
...
2. **Argument deltas.** Zero or more `delta` events follow, each carrying a `partial_tool_call` whose `arguments` is the next fragment of the arguments JSON string (`name` and `tool_call_id` are empty `""`). Concatenate the fragments in order. A provider that streams several calls at once tags each fragment with the `item_id` of its call; a fragment without one belongs to the call announced last.
...
`minimax-m3` is the one client that streams no `partial_tool_call` fragments: it reads each tool call from the server's completed output item and delivers step 3 alone, so nothing it streamed can disagree with the complete call. The `tool_call` item and the `ToolCallArgumentParseError` rule above apply unchanged.
