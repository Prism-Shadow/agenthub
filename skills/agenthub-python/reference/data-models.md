...
2. **Argument deltas.** Zero or more `delta` events follow, each carrying a `partial_tool_call` whose `arguments` is the next fragment of the arguments JSON string (`name` is empty `""`; `tool_call_id` names the call the fragment belongs to, or is empty `""` when the provider does not identify fragments, in which case the fragment belongs to the call announced last). Concatenate the fragments in order.
